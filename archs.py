...
from kan_bernstein import KANLinear, KAN
from kan import KANLinearS, KANS
from torch.nn import init
import torchvision.models as models
from torch.nn.init import trunc_normal_

__all__ = ['KANLayer', 'KANBlock', 'DwConv', 'Dw_bn_relu', 'PatchEmbed', 'ConvLayer', 'D_ConvLayer', 'UKAB']
...
class KANBlock(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # Reshape for SE and apply attention
        x_reshaped = x.permute(0, 2, 1).view(B, C, H, W)
        x_reshaped = self.se(x_reshaped)
        x = x_reshaped.view(B, C, -1).permute(0, 2, 1)
        
        x_norm = self.norm2(x)
        for layer in self.layer:
            x_norm = layer(x_norm, H, W) if isinstance(layer, (KANLayer, KANLayerS)) else layer(x_norm)
        
        x = x + self.drop_path(x_norm)
